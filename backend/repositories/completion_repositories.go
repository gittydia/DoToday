...
func (r *CompletionRepository) Create(completion *models.Completion) error {
	query := `
		INSERT INTO completions (id, goal_id, date, created_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (goal_id, date) DO NOTHING
	`
	_, err := r.db.Exec(query,
		completion.ID, completion.GoalID, completion.Date, completion.CreatedAt,
	)
	return err
}

func (r *CompletionRepository) GetByGoalID(goalID uuid.UUID) ([]*models.Completion, error) {
	query := `
		SELECT id, goal_id, date, created_at
		FROM completions
		WHERE goal_id = $1
		ORDER BY date DESC
	`
...
func (r *CompletionRepository) GetCompletionExists(goalID uuid.UUID, date time.Time) (bool, error) {
	var exists bool
...
func (r *CompletionRepository) CalculateCurrentStreak(goalID uuid.UUID) (int, error) {
	query := `
...
func (r *CompletionRepository) GetCompletionGraphData(goalID uuid.UUID, days int) ([]models.CompletionGraphData, error) {
	query := `
		WITH date_series AS (
